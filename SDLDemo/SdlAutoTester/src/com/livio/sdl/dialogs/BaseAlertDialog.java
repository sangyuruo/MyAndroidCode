...
/**
 * An abstract base class for custom alert dialogs. The class is basically just
 * a wrapper using Android's AlertDialog.Builder class to create relatively
...
public abstract class BaseAlertDialog {
	...
	/**
	 * Allows another class to listen for the result of the subclass dialog.
	 * Results are returned as an object.
	 *
	 * @author Mike Burke
	 *
	 */
	public interface Listener {
		 ...
		 * 
		 * @param resultData
		 *            The data results for the dialog
		 */
		void onResult(Object resultData);
	}

	/**
	 * Called after views have been inflated, allowing the subclass an
	 * opportunity to find their custom views using view.findViewById(id).
	 * 
	 * @param parent
	 *            The parent view which was inflated from input resource id.
	 */
	protected abstract void findViews(View parent);

	// since this is an abstract class, we'll set all variables to protected
	// scope in case subclasses want to use them directly
	 ...
	 * @param resource
	 *            The resource id used for inflating the main view of the dialog
	 */
	...
	protected void createDialog() {
		...
		builder.setTitle(title).setView(view).setCancelable(cancelable);
	 ...
	 * 
	 * @param resourceId
	 *            The id of the XML view to inflate
	 */
	 ...
	 * 
	 * @param cancelable
	 *            True if this dialog is cancelable, false if not
	 */
	 ...
	 * 
	 * @param resultData
	 *            Object containing dialog results
	 */
