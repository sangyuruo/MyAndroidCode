...
public class SdlConnectionDialog extends BaseOkCancelDialog {
	...
	public SdlConnectionDialog(Context context, int transportType,
			String initIpAddress, String initPort) {
		this(context);
		RadioButton initButton = (transportType == LivioSdlTesterPreferences.PREF_TRANSPORT_BLUETOOTH) ? radio_bt
				: radio_wifi;
	...
	protected void findViews(View parent) {
		...
		radio_transportGroup = (RadioGroup) parent
				.findViewById(R.id.radio_transportGroup);
	...
	private final DialogInterface.OnClickListener okButtonListener = new DialogInterface.OnClickListener() {
		...
		public void onClick(DialogInterface dialog, int which) {
			...
			if (selectedId == R.id.radio_bt) {
				...
				ipPort = null;
				connType = 2;
			} else if (selectedId == R.id.radio_usb) {
				ipAddress = null;
				ipPort = null;
				connType = 3;
			...
			IpAddress result = new IpAddress(ipAddress, ipPort, connType);
