...
public class WiProProtocol extends AbstractProtocol {
	...
	public void setVersion(byte version) {
		if (version > 4) {
			this._version = 4; // protect for future, proxy only supports v4 or
								// lower
			HEADER_SIZE = 12;
			MAX_DATA_SIZE = V1_V2_MTU_SIZE - HEADER_SIZE; // default to lowest
															// size since
															// capabilities of
															// this version are
															// unknown
		...
		LogUtil.debugSDLTrans("_version is " + _version);
	}

	public void StartProtocolSession(SessionType sessionType) {
		LogUtil.debugSDLTrans("createStartSession , _version is " + _version);
		ProtocolFrameHeader header = ProtocolFrameHeaderFactory
				.createStartSession(sessionType, 0x00, _version, (byte) 0x00);
		sendFrameToTransport(header);
	} // end-method

	private void sendStartProtocolSessionACK(SessionType sessionType,
			byte sessionID) {
		LogUtil.debugSDLTrans("createStartSessionACK , _version is " + _version);
		ProtocolFrameHeader header = ProtocolFrameHeaderFactory
				.createStartSessionACK(sessionType, sessionID, 0x00, _version);
		sendFrameToTransport(header);
	} // end-method

	public void EndProtocolSession(SessionType sessionType, byte sessionID) {
		LogUtil.debugSDLTrans("createEndSession , _version is " + _version);
		ProtocolFrameHeader header = ProtocolFrameHeaderFactory
				.createEndSession(sessionType, sessionID, hashID, _version);
	...
	public void SendMessage(ProtocolMessage protocolMsg) {
		...
		if (_version > 1 && sessionType != SessionType.NAV
			...
			if (protocolMsg.getBulkData() != null) {
				data = new byte[12 + protocolMsg.getJsonSize()
						+ protocolMsg.getBulkData().length];
				sessionType = SessionType.BULK_DATA;
			} else
				data = new byte[12 + protocolMsg.getJsonSize()];
			BinaryFrameHeader binFrameHeader = new BinaryFrameHeader();
			binFrameHeader = ProtocolFrameHeaderFactory
					.createBinaryFrameHeader(protocolMsg.getRPCType(),
							protocolMsg.getFunctionID(),
							protocolMsg.getCorrID(), protocolMsg.getJsonSize());
			System.arraycopy(binFrameHeader.assembleHeaderBytes(), 0, data, 0,
					12);
			System.arraycopy(protocolMsg.getData(), 0, data, 12,
					protocolMsg.getJsonSize());
			if (protocolMsg.getBulkData() != null) {
				System.arraycopy(protocolMsg.getBulkData(), 0, data,
						12 + protocolMsg.getJsonSize(),
						protocolMsg.getBulkData().length);
			}
		} else {
		...
		if (messageLock == null) {
			handleProtocolError(
					"Error sending protocol message to SDL.",
					new SdlException(
							"Attempt to send protocol message prior to startSession ACK.",
							SdlExceptionCause.SDL_UNAVAILABLE));
			return;
		}
		...
		synchronized (messageLock) {
			if (data.length > MAX_DATA_SIZE) {
				...
				messageID++;
				ProtocolFrameHeader firstHeader = ProtocolFrameHeaderFactory
						.createMultiSendDataFirst(sessionType, sessionID,
								messageID, _version);
				...
				System.arraycopy(BitConverter.intToByteArray(data.length), 0,
						firstFrameData, 0, 4);
				// Second four bytes are frame count.
				System.arraycopy(BitConverter.intToByteArray(frameCount), 0,
						firstFrameData, 4, 4);

				handleProtocolFrameToSend(firstHeader, firstFrameData, 0,
						firstFrameData.length);
				...
				for (int i = 0; i < frameCount; i++) {
					...
					ProtocolFrameHeader consecHeader = ProtocolFrameHeaderFactory
							.createMultiSendDataRest(sessionType, sessionID,
									bytesToWrite, frameSequenceNumber,
									messageID, _version);
					handleProtocolFrameToSend(consecHeader, data,
				...
				messageID++;
				ProtocolFrameHeader header = ProtocolFrameHeaderFactory
						.createSingleSendData(sessionType, sessionID,
								data.length, messageID, _version);
				handleProtocolFrameToSend(header, data, 0, data.length);
			}
		}
	}
	...
	public void HandleReceivedBytes(byte[] receivedBytes,
		...
		byte[] remainingBytes = processReceivedBytes(receivedBytes,
				receivedBytesLength);
		while (remainingBytes != null) {
			remainingBytes = processReceivedBytes(remainingBytes,
					remainingBytes.length);
		}
	}

	private byte[] processReceivedBytes(byte[] receivedBytes,
			int receivedBytesLength) {
		...
		if (!_haveHeader) {
			...
			if (receivedBytesLength < headerBytesNeeded) {
				...
				_haveHeader = true;
				_currentHeader = ProtocolFrameHeader
						.parseWiProHeader(_headerBuf);
				...
				if (iDataSize <= MAX_DATA_SIZE) {
					...
					Log.e("HandleReceivedBytes",
							"Corrupt header found, request to allocate a byte array of size: "
									+ iDataSize);
					Log.e("HandleReceivedBytes",
							"_headerBuf: " + _headerBuf.toString());
					Log.e("HandleReceivedBytes", "_currentHeader: "
							+ _currentHeader.toString());
					Log.e("HandleReceivedBytes", "receivedBytes: "
							+ receivedBytes.toString());
					Log.e("HandleReceivedBytes", "receivedBytesReadPos: "
							+ receivedBytesReadPos);
					Log.e("HandleReceivedBytes", "_headerBufWritePos: "
							+ _headerBufWritePos);
					Log.e("HandleReceivedBytes", "headerBytesNeeded: "
							+ headerBytesNeeded);
					handleProtocolError(
							"Error handling protocol message from sdl, header invalid.",
							new SdlException(
									"Error handling protocol message from sdl, header invalid.",
									SdlExceptionCause.INVALID_HEADER));
					return null;
				}
				_dataBufWritePos = 0;
			}
		...
		if (_dataBuf == null) {
			Log.e("HandleReceivedBytes",
					"Error: Databuffer is null, logging debug info.");
			try {
				Log.e("HandleReceivedBytes",
						"_headerBuf: " + _headerBuf.toString());
				Log.e("HandleReceivedBytes", "_currentHeader: "
				...
				Log.e("HandleReceivedBytes", "receivedBytesReadPos: "
						+ receivedBytesReadPos);
				Log.e("HandleReceivedBytes", "receivedBytesLength: "
						+ receivedBytesLength);
				Log.e("HandleReceivedBytes", "_headerBufWritePos: "
						+ _headerBufWritePos);
			...
			handleProtocolError(
					"Error handling protocol message from sdl, header invalid.",
					new SdlException(
							"Error handling protocol message from sdl, data buffer is null.",
							SdlExceptionCause.DATA_BUFFER_NULL));
			return null;
		}

		onResetIncomingHeartbeat(_currentHeader.getSessionType(),
				_currentHeader.getSessionID());
	...
	protected MessageFrameAssembler getFrameAssemblerForFrame(
			ProtocolFrameHeader header) {
		Hashtable<Integer, MessageFrameAssembler> hashSessionID = _assemblerForSessionID
				.get(Byte.valueOf(header.getSessionID()));
		if (hashSessionID == null) {
			hashSessionID = new Hashtable<Integer, MessageFrameAssembler>();
			_assemblerForSessionID.put(Byte.valueOf(header.getSessionID()),
					hashSessionID);
		} // end-if

		MessageFrameAssembler ret = (MessageFrameAssembler) _assemblerForMessageID
				.get(Integer.valueOf(header.getMessageID()));
		if (ret == null) {
			ret = new MessageFrameAssembler();
			_assemblerForMessageID.put(Integer.valueOf(header.getMessageID()),
					ret);
		} // end-if
	...
	protected class MessageFrameAssembler {
		...
		protected void handleFirstDataFrame(ProtocolFrameHeader header,
				byte[] data) {
		...
		protected void handleRemainingFrame(ProtocolFrameHeader header,
				byte[] data) {
		...
		protected void notifyIfFinished(ProtocolFrameHeader header) {
			...
			if (header.getFrameType() == FrameType.Consecutive
					&& header.getFrameData() == 0x0) {
				...
				try {
					...
					DebugTool.logError(FailurePropagating_Msg
							+ "onProtocolMessageReceived: " + excp.toString(),
							excp);
				} // end-catch
		...
		protected void handleMultiFrameMessageFrame(ProtocolFrameHeader header,
				byte[] data) {
			// if (!hasFirstFrame) {
			// hasFirstFrame = true;
		...
		protected void handleFrame(ProtocolFrameHeader header, byte[] data) {
			if (header.getFrameType().equals(FrameType.Control)) {
				...
				// Must be a form of data frame (single, first, consecutive,
				// etc.)
				if (header.getFrameType() == FrameType.First
		...
		private void handleProtocolHeartbeat(ProtocolFrameHeader header,
				byte[] data) {
			WiProProtocol.this.handleProtocolHeartbeat(header.getSessionType(),
					header.getSessionID());
		} // end-method
		...
		private void handleProtocolHeartbeatACK(ProtocolFrameHeader header,
			...
			WiProProtocol.this.handleProtocolHeartbeatACK(
					header.getSessionType(), header.getSessionID());
		} // end-method

		private void handleControlFrame(ProtocolFrameHeader header, byte[] data) {
			if (header.getFrameData() == FrameDataControlFrameType.Heartbeat
					.getValue()) {
				handleProtocolHeartbeat(header, data);
			} else if (header.getFrameData() == FrameDataControlFrameType.HeartbeatACK
					.getValue()) {
				handleProtocolHeartbeatACK(header, data);
			} else if (header.getFrameData() == FrameDataControlFrameType.StartSession
					.getValue()) {
				sendStartProtocolSessionACK(header.getSessionType(),
						header.getSessionID());
			} else if (header.getFrameData() == FrameDataControlFrameType.StartSessionACK
					.getValue()) {
			...
			} else if (header.getFrameData() == FrameDataControlFrameType.StartSessionNACK
					.getValue()) {
				if (header.getSessionType().eq(SessionType.NAV)
						|| header.getSessionType().eq(SessionType.PCM)) {
					handleProtocolSessionNACKed(header.getSessionType(),
							header.getSessionID(), _version, "");
				} else {
					handleProtocolError(
							"Got StartSessionNACK for protocol sessionID="
									+ header.getSessionID(), null);
				}
			} else if (header.getFrameData() == FrameDataControlFrameType.EndSession
					.getValue()) {
				// if (hashID == BitConverter.intFromByteArray(data, 0))
				if (_version > 1) {
					if (hashID == header.getMessageID())
						handleProtocolSessionEnded(header.getSessionType(),
								header.getSessionID(), "");
				} else
					handleProtocolSessionEnded(header.getSessionType(),
							header.getSessionID(), "");
			} else if (header.getFrameData() == FrameDataControlFrameType.EndSessionACK
					.getValue()) {
				handleProtocolSessionEnded(header.getSessionType(),
						header.getSessionID(), "");
			} else if (header.getFrameData() == FrameDataControlFrameType.EndSessionNACK
					.getValue()) {
				handleProtocolSessionEndedNACK(header.getSessionType(),
						header.getSessionID(), "");
			} else if (header.getFrameData() == FrameDataControlFrameType.ServiceDataACK
					.getValue()) {
				handleProtocolServiceDataACK(header.getSessionType(),
						header.getSessionID());
			}

		} // end-method

		private void handleSingleFrameMessageFrame(ProtocolFrameHeader header,
				byte[] data) {
			ProtocolMessage message = new ProtocolMessage();
			...
			try {
				...
				DebugTool.logError(FailurePropagating_Msg
						+ "onProtocolMessageReceived: " + ex.toString(), ex);
				handleProtocolError(FailurePropagating_Msg
						+ "onProtocolMessageReceived: ", ex);
			} // end-catch
		} // end-method
	} // end-class
	...
	public void StartProtocolService(SessionType sessionType, byte sessionID) {
		ProtocolFrameHeader header = ProtocolFrameHeaderFactory
				.createStartSession(sessionType, 0x00, _version, sessionID);
		sendFrameToTransport(header);
	...
	public void SendHeartBeat(byte sessionID) {
		final ProtocolFrameHeader heartbeat = ProtocolFrameHeaderFactory
				.createHeartbeat(SessionType.CONTROL, sessionID, _version);
	...
	public void SendHeartBeatACK(byte sessionID) {
		final ProtocolFrameHeader heartbeat = ProtocolFrameHeaderFactory
				.createHeartbeatACK(SessionType.CONTROL, sessionID, _version);
		sendFrameToTransport(heartbeat);
	}
} // end-class